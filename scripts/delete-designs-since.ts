 ...
 *     [--until <ISO-8601>] [--apply] [--confirm-prod]
 *
 ...
 * inline creds for prod/preview. The target is classified and printed; a prod
 * target additionally requires `--confirm-prod` alongside `--apply`. R2 objects
 * of deleted images are removed on apply (the bucket is shared across envs, so
 * the R2 creds come from .env.local either way).
 *
...
import { deleteDesignsSince } from "../src/lib/delete-designs-since";
...
function usage(msg?: string): never {
  ...
  console.error(
    [
      ...
      "  --since         inclusive lower bound on design.created_at",
      "  --until         inclusive upper bound (default: now)",
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--apply" || a === "--confirm-prod") {
      flags.add(a);
    } else if (a === "--user" || a === "--since" || a === "--until") {
      const v = argv[i + 1];
...
function parseDate(flag: string, raw: string): Date {
  ...
  if (Number.isNaN(d.getTime())) usage(`${flag} is not a valid ISO-8601 date: ${raw}`);
  return d;
}
...
async function main() {
  ...
  console.log(
    `${apply ? "APPLY" : "DRY RUN"} — ${email}, created ${since.toISOString()} … ${(until ?? new Date()).toISOString()}`
  );
  console.log("");
