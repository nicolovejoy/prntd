 ...
 *
 * PRE-0014 (`listing` present) — Nico's PRE-CHECK before applying 0014. Proves
 ...
 *
 * POST-0014 (`image_publication` present) — Nico's VERIFY after applying.
 ...
 * missing; `__drizzle_migrations` has no row at or after 0014's journal
 ...
 * verify a drop migration — this script is the verify step for 0014.
 *
...
async function main() {
  ...
  if (report.problems.length) {
    ...
    console.error(
      report.mode === "pre-0014"
        ? "\nDo NOT apply migration 0014 until every problem above is resolved." +
        ...
        : "\nMigration 0014 did not land as written — stop and compare against drizzle/0014_thin_pride.sql."
    );
    process.exit(1);
  }
  ...
  console.log(
    report.mode === "pre-0014"
      ? "\nPRE-0014 CHECK CLEAN — safe to apply migration 0014"
      : "\nPOST-0014 VERIFY CLEAN"
  );
}
