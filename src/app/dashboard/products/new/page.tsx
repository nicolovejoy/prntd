...
function NewProductForm() {
  ...
  const [blankId, setBlankId] = useState<string>(ACTIVE_BLANKS[0].id);
  const [size, setSize] = useState("M");
  const [color, setColor] = useState<string>(ACTIVE_BLANKS[0].colors[0].name);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const ds = await getComposableDesigns();
        setDesigns(ds);
        if (ds[0]) setDesignId(ds[0].designId);
      } catch {
        setDesigns([]);
      }
    })();
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <section className="mt-5">
        ...
        {designs === null && <p className="text-sm text-text-faint">Loading…</p>}
        {designs?.length === 0 && (
