...
const COST_PER_GENERATION = 0.03; // approximate Flux cost
...
export async function sendMessage(designId: string, userMessage: string) {
  ...
  // Get Claude to construct the Flux prompt
  const aiResponse = await constructFluxPrompt(userMessage, chatHistory);

  // Generate image with Flux
  const imageUrl = await generateImage(aiResponse.fluxPrompt);

  // Download and upload to R2
  const imageResponse = await fetch(imageUrl);
  const imageBuffer = Buffer.from(await imageResponse.arrayBuffer());
  const newGeneration = found.generationCount + 1;
  const r2Url = await uploadDesignImage(designId, newGeneration, imageBuffer);
