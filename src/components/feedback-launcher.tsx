"use client";

import { useState } from "react";
import { FeedbackWidget } from "./feedback-widget";

// Fixed bottom-right launcher. Tap to toggle a small feedback panel that
// POSTs to ibuild4you. Rendered globally from the root layout so the page
// URL captured in each submission reflects wherever the user was.
export function FeedbackLauncher({ projectId }: { projectId: string }) {
  const [open, setOpen] = useState(false);

  return (
    <div className="fixed bottom-4 right-4 z-50 print:hidden">
      {open && (
        <div className="mb-2 w-72 max-w-[calc(100vw-2rem)] rounded-lg border border-border bg-surface-raised p-4 shadow-lg">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm font-medium text-foreground">Feedback</span>
            <button
              type="button"
              onClick={() => setOpen(false)}
              aria-label="Close feedback"
              className="text-text-muted hover:text-foreground"
            >
              ✕
            </button>
          </div>
          <FeedbackWidget projectId={projectId} onSent={() => setOpen(false)} />
        </div>
      )}

      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="rounded-full border border-border bg-surface-raised px-4 py-2 text-sm text-text-muted shadow-lg transition-colors hover:border-border-hover hover:text-foreground"
      >
        {open ? "Close" : "Feedback"}
      </button>
    </div>
  );
}
