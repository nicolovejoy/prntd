/**
 * Composition parity check, dual mode around migration 0014 (composition
 ...
 * pre-0014 shape (`listing`, `product.store_id` / `design_id`) and the
 * post-0014 shape (`image_publication`, generated `product.front_image_id`),
 * and Drizzle's schema only knows the latter.
 *
 * The mode is read off `sqlite_master`:
 *
 *   - `image_publication` present  → post-0014: Nico's VERIFY after migrating.
 *   - else `listing` present       → pre-0014: Nico's PRE-CHECK before migrating.
 *   - neither                      → not a prntd database; throws.
 *
 * `scripts/migration-smoke.ts` exits 1 on any dropped table, so it cannot
 * verify a drop migration; this check is the verify step for 0014.
 */
import { readFileSync } from "node:fs";
import { join } from "node:path";
import type { Client, Row } from "@libsql/client";

export type CompositionParityMode = "pre-0014" | "post-0014";
 ...
 * skips entries whose `when` is ≤ the newest row — so "0014 has been applied"
 * is "a ledger row with created_at ≥ 0014's when", and a pre-0014 database
 * that is really at 0013 has its newest row AT 0013's when. Read at module
...
export const MIGRATION_0014_WHEN: number = journalWhen(/^0014_/);

/** The visibility grant, reduced: exactly these four columns, in this order, after 0014. */
 ...
 * The pre-0014 caller passes the surviving mirror rows (store_id and
 * design_id both NULL); the post-0014 caller passes every product row, since
...
/**
 * PRE-0014: proves the migration will (a) run — its guard and its new unique
 * index cannot fail — and (b) delete only what it is meant to. Mirrors the
 * three data-dependent statements in drizzle/0014_thin_pride.sql.
 */
async function checkPre(
  ...
  for (const t of ["product", "order"]) {
    if (!tables.has(t)) {
      throw new Error(`pre-0014 database is missing table ${t} — not a prntd database`);
    }
  }
  ...
  for (const t of ["__slice5_guard", "__new_product"]) {
    if (tables.has(t)) {
      problems.push(
        `table ${t} already exists — migration 0014 creates it as a scratch table and would fail on CREATE TABLE; drop the leftover by hand first`
      );
    }
  }
  ...
  if (badJson.length) {
    problems.push(
      `${badJson.length} product row(s) have malformed placements JSON — migration 0014's unique index cannot evaluate them (it would fail after the drops and roll back); fix by hand first: ${badJson
        .map((r) => String(r.id))
        .join(", ")}`
    );
  }
  ...
  if (storeOrders.length) {
    problems.push(
      `${storeOrders.length} order(s) carry store_id — migration 0014's guard will refuse to run (dropping order.store_id would orphan an organizer-storefront sale): ${storeOrders
  ...
  if (organizerOrders.length) {
    problems.push(
      `${organizerOrders.length} order(s) reference an organizer product via store_product_id — migration 0014's guard will refuse to run (its DELETE would remove that product from under the order): ${organizerOrders
  ...
  for (const [front, ids] of byFront) {
    if (ids.length > 1) {
      duplicateFronts++;
      problems.push(
        `front image ${front} is pinned by ${ids.length} surviving compositions (${ids.join(", ")}) — migration 0014's unique index product_front_image_unique cannot be built; dedupe by hand first`
      );
    }
  }
  ...
  if (organizer.length) {
    notes.push(
      `${organizer.length} organizer product row(s) will be DELETED by migration 0014 (design_id or store_id set; test-era — the guard refuses the migration if any order still references one):`
    );
    ...
    notes.push("no organizer product rows — migration 0014's DELETE is a no-op here");
  }
  ...
  // actually at 0013 gets a wider batch than "just 0014".
  if (tables.has("__drizzle_migrations")) {
    const last = num(
      (await client.execute("select max(created_at) as n from `__drizzle_migrations`")).rows[0].n
    );
    notes.push(
      last === MIGRATION_0013_WHEN
        ? `migrations ledger: newest created_at ${last} = 0013's journal when — db:migrate will apply exactly 0014`
        : `migrations ledger: newest created_at ${last ?? "none"} ≠ 0013's journal when ${MIGRATION_0013_WHEN} — db:migrate applies every journal entry newer than that row, not just 0014`
    );
  } else {
    notes.push(
...
/**
 * POST-0014: proves the migration landed as written — every drop, the rename
...
async function checkPost(
  ...
  for (const gone of ["store", "product_offering", "listing", "__slice5_guard", "__new_product"]) {
    if (tables.has(gone)) problems.push(`table ${gone} still exists — migration 0014 should have dropped it`);
  }
  for (const t of ["product", "order"]) {
    if (!tables.has(t)) {
      throw new Error(`post-0014 database is missing table ${t} — not a prntd database`);
    }
  }
  ...
  if (orderCols.includes("store_id")) {
    problems.push("order.store_id still exists — migration 0014 should have dropped it");
  }

  const productCols = await columnNames(client, "product");
  for (const col of ["store_id", "design_id"]) {
    if (productCols.includes(col)) {
      problems.push(`product.${col} still exists — migration 0014 should have dropped it`);
    }
  }
  ...
  if (!tables.has("__drizzle_migrations")) {
    problems.push(
      "no __drizzle_migrations table — this database has never been migrated by drizzle-kit; 0014 cannot have been applied through it"
    );
  } else {
    const applied = await count(
      client,
      `select count(*) as n from __drizzle_migrations where created_at >= ${MIGRATION_0014_WHEN}`
    );
    if (applied === 0) {
      problems.push(
        `__drizzle_migrations has no row with created_at >= ${MIGRATION_0014_WHEN} (0014's journal when) — the migrator does not consider 0014 applied`
      );
    }
  }
