 ...
 * (#191): migration 0014 deleted the test-era rows and dropped both columns.
 *
 ...
 * schema-derived test DB.
 *
 * The expression is `placements ->> '$.front'` (SQLite's JSON arrow operator,
 * same value as `json_extract(placements, '$.front')`) because drizzle-kit
 * 0.31's SQLite introspection reads a generated expression only up to its
 * FIRST closing parenthesis. A nested call like `json_extract(…)` reads back
 * truncated, so every `db:push` against a migrated database proposed dropping
 * and re-adding the column (and the drop fails on the unique index). The arrow
 * form has no inner parentheses and reads back exactly. Re-parented on the
 * guest→account claim via `ownerId`.
 */
 ...
 * migration 0014. `imageId` is opaque text (no FK).
 */
