 ...
 * column directly. `product` rows are the Shop compositions a published image
 * mirrors (composition slice 1) — owned via `product.ownerId`, so a guest who
 * published before signing up keeps the mirror. The Model B `image` table
 * denormalizes owner_id, so it re-parents directly here (placement_render /
 * conversation_image follow via design_id, image_publication via its image).
