 ...
 * migration chain — the pre-0014 shape (0000–0013, which the schema-derived
 * test-db.ts can never build) and the post-0014 shape (0000–0014).
 *
 * The SQL files are replayed statement-by-statement as the 0006 and 0014
 * tests do. Replaying THROUGH 0014 that way needs `PRAGMA foreign_keys=OFF`
...
/** Replay the chain up to (and, when `through0014`, including) migration 0014. */
async function chainDb(through0014: boolean): Promise<Client> {
  const client = createClient({ url: ":memory:" });
  await client.execute("PRAGMA foreign_keys=OFF");
  const files = migrationFiles();
  const target = files.find((f) => /^0014_/.test(f))!;
  const end = files.indexOf(target) + (through0014 ? 1 : 0);
...
/** A pre-0014 Shop composition for `front`, listed at `listedAt`. */
...
describe("checkCompositionParity — pre-0014 (Nico's pre-check)", () => {
  let client: Client;
  ...
  it("clean: listing and mirror agree, no organizer rows, no store orders", async () => {
    ...
    expect(report.mode).toBe("pre-0014");
    ...
    expect(report.notes).toEqual([
      "no organizer product rows — migration 0014's DELETE is a no-op here",
  ...
  it("a leftover scratch table from an interrupted run is a problem", async () => {
    ...
    const report = await checkCompositionParity(client);
    expect(report.problems).toEqual([
      "table __new_product already exists — migration 0014 creates it as a scratch table and would fail on CREATE TABLE; drop the leftover by hand first",
    ]);
  });
  ...
  it("reports where the migrator thinks the database is", async () => {
    ...
    expect(at12.notes).toContain(
      `migrations ledger: newest created_at ${MIGRATION_0013_WHEN} = 0013's journal when — db:migrate will apply exactly 0014`
    );
    ...
    expect(behind.notes.some((n) => /≠ 0013's journal when .* not just 0014/.test(n))).toBe(true);
  ...
  it("an order carrying store_id is a problem (guard statement 1)", async () => {
    ...
    const report = await checkCompositionParity(client);
    expect(report.problems).toHaveLength(1);
    expect(report.problems[0]).toMatch(/^1 order\(s\) carry store_id — migration 0014's guard will refuse to run/);
  ...
  it("an order whose store_product_id is an organizer product is a problem (guard statement 2)", async () => {
    ...
    expect(report.problems[0]).toMatch(
      /^1 order\(s\) reference an organizer product via store_product_id — migration 0014's guard will refuse to run/
    );
  ...
  it("two surviving rows sharing a front image is a problem (the unique index could not be built)", async () => {
    ...
    expect(report.problems[0]).toBe(
      "front image img1 is pinned by 2 surviving compositions (mir, mir2) — migration 0014's unique index product_front_image_unique cannot be built; dedupe by hand first"
    );
  ...
  it("organizer rows the migration will delete appear as notes with a count, even when they share the mirror's front", async () => {
    ...
    expect(report.notes.filter((n) => !/^(migrations ledger|no __drizzle_migrations)/.test(n))).toEqual([
      "2 organizer product row(s) will be DELETED by migration 0014 (design_id or store_id set; test-era — the guard refuses the migration if any order still references one):",
...
describe("checkCompositionParity — post-0014 (Nico's verify)", () => {
  let client: Client;

  beforeEach(async () => {
    client = await chainDb(true);
    await ledger(client, MIGRATION_0014_WHEN);
    await baseRows(client);
    await client.execute(
  ...
  it("clean: drops landed, generated column + unique index present, ledger stamped, rows agree", async () => {
    const report = await checkCompositionParity(client);
    expect(report.mode).toBe("post-0014");
    expect(report.problems).toEqual([]);
    expect(report.notes).toEqual([]);
  ...
  it("a later ledger row (0015 and beyond) still counts as 0014 applied", async () => {
    await client.execute("DELETE FROM `__drizzle_migrations`");
    await ledger(client, MIGRATION_0014_WHEN + 100_000);
    const report = await checkCompositionParity(client);
    expect(report.problems).toEqual([]);
  });

  it("a missing 0014 ledger row is a problem", async () => {
    await client.execute("DELETE FROM `__drizzle_migrations`");
    await ledger(client, MIGRATION_0014_WHEN - 1);
    const report = await checkCompositionParity(client);
    expect(report.problems).toEqual([
...
describe("checkCompositionParity — mode detection", () => {
  ...
  it("reads 0014's `when` from the journal", () => {
    expect(MIGRATION_0014_WHEN).toBeGreaterThan(1_700_000_000_000);
  });
});
