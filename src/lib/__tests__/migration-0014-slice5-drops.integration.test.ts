...
/**
 * Migration 0014 (composition slice 5 + #191 step 2) against a real libSQL,
 ...
 * table at the top of 0014 relies on exactly that, so the guard cases below
 * have to go through `migrate()` (the same code path `npm run db:migrate`
 * takes), not through a hand-rolled loop.
 *
 * Setup: 0000–0013 replayed statement-by-statement (the pre-slice-5 shape,
 * which the schema-derived test-db.ts can never build), the migrator's own
 * `__drizzle_migrations` ledger stamped at 0013's journal `when`, and a
 * migrations folder truncated at 0014 — so the migrator applies exactly 0014
...
/** Statement-by-statement replay, as the 0006 test does — for the PRE-0014 chain only. */
...
/**
 * A migrations folder the migrator will read that ends at 0014: the real
 * journal truncated after the 0014 entry, plus the SQL files it names. Keeps
 * this test pinned to 0014 when 0015+ exist.
 */
function migrationsFolderEndingAt0014(): string {
  const journal = readJournal();
  const end = journal.entries.findIndex((e) => TARGET.test(e.tag));
  if (end === -1) throw new Error("no 0014 entry in drizzle/meta/_journal.json");
  const dir = mkdtempSync(join(tmpdir(), "prntd-migration-0014-"));
...
/**
 * Pre-0014 fixture, in the shapes the drops care about:
 ...
 *  - an abandoned checkout (#234's `order.abandoned_at`, added by 0013 — the
 *    migration right before this one), whose timestamp must survive the
 *    `order.store_id` drop.
 */
async function seed(client: Client) {
  ...
  await run(
    `INSERT INTO \`order\` (\`id\`, \`user_id\`, \`design_id\`, \`total_price\`, \`status\`, \`stripe_session_id\`, \`abandoned_at\`, \`created_at\`)
     VALUES ('oa', 'u', 'd', 24.12, 'pending', 'cs_test_abandoned', 1788900000, 0)`
  );
}

describe("migration 0014 — composition slice 5 drops, through the real migrator", () => {
  let client: Client;
  let folder: string;
  ...
  beforeEach(async () => {
    ...
    // creates it, with one row stamped at 0013's journal `when`. The migrator
    // applies every journal entry whose `when` is greater — with the folder
    // truncated at 0014, that is 0014 alone.
    await client.execute(
      'CREATE TABLE IF NOT EXISTS "__drizzle_migrations" (id SERIAL PRIMARY KEY, hash text NOT NULL, created_at numeric)'
    );
    const journal = readJournal();
    const prev = journal.entries.find((e) => /^0013_/.test(e.tag));
    if (!prev) throw new Error("no 0013 entry in the journal");
    await client.execute({
      sql: 'INSERT INTO "__drizzle_migrations" ("hash", "created_at") VALUES (?, ?)',
      args: ["seeded-0013", prev.when],
    });
    expect(await ledgerCount(client)).toBe(1);

    folder = migrationsFolderEndingAt0014();
    await seed(client);
  });
  ...
  it("applies clean: drops, rename, recreate, unique index, and the ledger row", async () => {
    ...
    // 0013's abandoned_at survives the drop, value intact.
    expect(await columns(client, "order")).toContain("abandoned_at");
    const abandoned = await client.execute(
      "SELECT `status`, `stripe_session_id`, `abandoned_at` FROM `order` WHERE `id` = 'oa'"
    );
    expect(abandoned.rows[0]).toMatchObject({
      status: "pending",
      stripe_session_id: "cs_test_abandoned",
      abandoned_at: 1788900000,
    });
  ...
  async function expectUntouched() {
    ...
    // No 0014 row: the batch — ledger insert included — rolled back.
    expect(await ledgerCount(client)).toBe(1);
  }
