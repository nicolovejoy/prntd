...
-- libSQL (see RECREATE below), so the file was replaced and the snapshot +
-- journal entry kept as drizzle-kit wrote them.
--
...
-- (the PR body carries the read-only pre-check).
--
...
-- `order.store_id` was added by migration 0002 with an inline REFERENCES, and
-- nothing since has recreated `order` (0006 and 0013 only ran ALTER TABLE
-- DROP/ADD COLUMN), so its plain DROP COLUMN works. The INSERT…SELECT lists the
-- surviving columns explicitly and every one of them exists on the OLD table,
-- so SQLite's double-quoted-string fallback — which turned unknown "columns"
-- into string LITERALS in the slice-1 generated SQL (0010) — cannot fire here.
-- The __new_product DDL is byte-identical (bar the name) to the CREATE TABLE
-- drizzle-kit derives from schema.ts, which is what the schema-derived test DB
-- and the drift gate both build.
--
-- GENERATED COLUMN. `front_image_id` = placements ->> '$.front' as a VIRTUAL
-- column plus an ordinary unique index on it is the DB-enforced "one
...
-- test-DB derivation. The arrow operator (same value as
-- json_extract(placements, '$.front'), SQLite 3.38+) rather than
-- json_extract(…) because drizzle-kit's SQLite introspection reads a generated
-- expression only up to its first ")": a nested call reads back truncated and
-- every later `db:push` would propose dropping and re-adding the column.
--
...
CREATE TABLE `__new_product` (
	...
	`front_image_id` text GENERATED ALWAYS AS (placements ->> '$.front') VIRTUAL,
