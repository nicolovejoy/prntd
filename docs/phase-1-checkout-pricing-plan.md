...
- **NEXT: schema migration** → then **1B real-time shipping quote + separate Stripe shipping line** (the margin fix; highest value). See steps below.
- **1C tax** = doc + nullable column only (defer collection). **1D multi-item** = model-only here; full cart is committed-soon as **#26**.
