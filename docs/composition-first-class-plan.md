...
**Slice 5 status (built 2026-09-06 as PR #201; rebuilt on main 2026-09-25 —
migration 0014 awaits Nico's hand-apply, see the PR body).** What shipped, and
where it differs from the paragraph above:
...
- The "one composition per front image" rule is a VIRTUAL generated column
  `product.front_image_id = placements ->> '$.front'` plus an ordinary unique
  index `product_front_image_unique` on it — not an expression index.
  drizzle-kit 0.31 splits an expression index on its comma and quotes the
  halves, which is invalid SQL in both the migration and the schema-derived
  test DB; the generated column also gives every reader a real column to join
  on (`composition-reads.ts` `mirrorFrontImageId`). The arrow operator (same
  value as `json_extract(placements, '$.front')`) because drizzle-kit's
  SQLite introspection reads a generated expression only up to its first
  `)`: the nested-call form read back truncated, so `db:push` against a
  migrated database proposed dropping and re-adding the column every time.
...
- The `"product"` bulk-delete skip reason was removed, and with it the
  "a product FKs this design" keeper in `removeDesignIfNowEmpty` (#243). With
  `product.design_id` gone a product can only PIN an image, and
  `imageReferences()` already treats a pin as detach, never block.
...
- Verification is `scripts/check-composition-read-parity.ts`, now dual-mode:
  pre-0014 it reports the guard's stop conditions, duplicate fronts, and the
  organizer rows the migration will delete; post-0014 it verifies every drop,
  the rename's column set, the generated column, the index, the 0014 ledger
