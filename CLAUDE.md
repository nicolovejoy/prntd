...
- **Next (slice-2 remainder):** store-edit UI (name/accent/slug), signed-in (non-anon) e2e helper + Playwright compose spec (`STORES_ENABLED` already in the harness), Printful mockup in the compose preview (currently artwork-on-color), product-edit page. Then **Phase 3** = `/shop/[slug]` storefront (Copy-link destination — currently 404s). Phase 4 = persona/copy with Manine. Branch pushed, not merged; no prod migrate until a UI phase flips the flag.
