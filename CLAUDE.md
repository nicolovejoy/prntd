...
**Migration discipline — PLANNED, NEXT BUILD (2026-06-13).** Execution checklist: `docs/migration-adoption-plan.md`; rationale + cross-project decision: `docs/cicd-roadmap.md` §"Migration discipline". prntd uses `db:push` (no versioned migrations — `drizzle/` empty); branch-per-env + creds already solid. Adopt Drizzle migrations: generate `0000` baseline, mark already-applied on prod (the only risky step — diff prod-vs-`schema.ts` first, dry-run on a throwaway Turso branch before touching prod). Keep prod migrate manual (byside parity, not fire-on-merge). Also make `.env.local` permanently dev-only + add a `DB_TARGET` op-resolver so prod ops are one clean command. 4 open Qs at the top of the plan doc — answer at session start. drizzle-kit@0.31.10, drizzle-orm@0.45.1 (verify subcommand syntax vs node_modules).
