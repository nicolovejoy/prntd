...
- **2026-07-04 multi-agent code review** (idempotency/architecture/coverage/test-quality) filed the debt backlog: **#37** money-path idempotency (atomic claim, batched ledger writes, `ledger_entry(order_id,type)` unique index, Printful `external_id`, Printful-webhook redelivery → 200 skipped, `onLinkAccount` batch, `checkoutCart` atomicity, promo×cart test matrix), **#38** cart cleared at Stripe-session creation instead of on payment (confirmed unintentional), **#39** cron retry for paid-but-unsubmitted orders + order naming off the payment path, **#40** generation races (R2 key collision on concurrent Generate, stale counters, quota test), **#41** test debt (seed factories, node env for lib tests, `orderBy` on order_item select, e2e data-testids, cart pricing unit tests). Architecture verdict: modular monolith, explicitly NOT microservices; lib import graph is acyclic; fulfillment is the extraction-ready seam if ever needed.
