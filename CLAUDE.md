...
- **2026-07-04 multi-agent code review** (idempotency/architecture/coverage/test-quality) filed the debt backlog #37–#41. Architecture verdict: modular monolith, explicitly NOT microservices; lib import graph is acyclic; fulfillment is the extraction-ready seam if ever needed. Status:
  ...
  - **Remaining, in build order:** **#40** generation races — R2 key collision on concurrent Generate, stale counters, quota test (3 independent fixes, good subagent fan-out in worktrees); **#41** test debt — seed factories, node env for lib tests, `orderBy` on order_item select, e2e data-testids (chat messages done in PR #46), cart pricing unit tests (mechanical, background-subagent batch); then data-model **Phase 1b** (always write `order_item`; also removes #38's single-item caveat) → **1c** (drop scalar cols).
