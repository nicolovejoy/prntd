...
- **Open UX concerns (persona-INDEPENDENT, shippable now — raised 2026-06-25):** (1) **Mobile buy-page layout** — `/d/[imageId]` (`src/app/d/[imageId]/page.tsx:34-76`) is one scrolling column with a full-width 1:1 image first, so the product/size/color picker + "Buy this design" CTA fall below the fold on a phone; no sticky CTA despite the buy-panel comment claiming one. Fix: cap image height on mobile + sticky bottom Buy CTA (matches `feedback_mobile_ux`). (2) **Inconsistent design-card destinations** — a Fresh Prints card (`/prints` → `published-grid` → `/d/[imageId]`, buy pipeline) vs a My-Designs card (`/designs` → `/design?id=`, chat thread, `designs/page.tsx:13`): same gesture, two different worlds. Partly the two-flow model by design, but the jarring landing needs clearer per-card affordances (View/Edit/Order) or a canonical view page. Both are the kind of mechanical fix shipped ahead of the Manine call (like Geist/tokens); persona-independent.
