...
- **Phase 1a DONE (2026-06-25):** pure `resolveOrderLines` normalizer (`src/lib/order-lines.ts`, 6 tests) unifies legacy scalar order cols + `order_item` rows → `OrderLine[]` (exposes `blankId`, the read-layer half of #2). Wired `/orders` (`getUserOrders`, per-line thumbnail+attribution, real-DB integration test — fixed a latent bug where multi-item cart orders showed only item 1) and `/order/confirm`. **Next:** admin order-detail display + order emails (CAUTION: admin's Printful re-submit path `admin/actions.ts`~L109 reads scalar fields for variant resolution — fulfillment, not a display change), then Phase 1b (always write `order_item`) → 1c (drop scalar cols).
