...
- **Phase 2 — back-of-shirt printing / multi-placement (#25) — IN PROGRESS.** Plan: `docs/phase-2-back-printing-plan.md`. `order.placements` already `Record<string,string>` → **zero DB migration**; runtime plumbing only. **Locked 2026-06-07:** not COGS-blocked on #11 (COGS comes from Printful's invoice; back upcharge is purely a pricing decision); back image **reused from the same design thread**; **opt-in** "Add a back design" on `/preview` (front stays required); upcharge rides the **product (discountable) Stripe line**, `BACK_PLACEMENT_COST` set from Printful's additional-placement fee (deferred to 2.3); mockups lazy/back-on-demand. Build behind `MULTI_PLACEMENT_ENABLED` (default off).
  ...
  - **NEXT — 2.4b (client UI) + tests.** Plan: `~/.claude/plans/precious-baking-mango.md`. (1) `/preview` Front/Back **toggle on the hero** (decided) + back-source picker via `getDesignGallery().sources` — placement-aware state refactor of `preview/page.tsx` (the risky part: replace single `mockupUrl` with `{front,back}`, add `activePlacement`/`backImageId`, render effect passes `backImageId` as `sourceImageId` for back). (2) `/order` read `?back=`, gate on `isMultiPlacementEnabled()`, show `Back design +$8.00` line, pass `back` to checkout. (3) integration test (front+back → two files + +$8 sale ledger). Then **2.5** real test-mode order → flip `MULTI_PLACEMENT_ENABLED`. Same machinery later delivers #17 (`label_inside`).
