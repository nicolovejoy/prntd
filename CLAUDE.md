...
**Maker landing ("Type It, Wear It") — PLANNED 2026-07-05, next build.** Plan: `docs/maker-landing-plan.md`; memory `project_positioning_organizer` (direction update). Landing becomes the composer: `/design?prompt=` seed auto-fires Draw-it (thin-check guards thin prompts), new `MakerHero` on the signed-out homepage, shared `EXAMPLES` module, honest proof strip from the feed, `minRetailPrice()` fixes the stale `$15` copy, "Open a shop →" quiet in the footer. One PR (`feat/maker-landing`). Chosen from the 4-concept reframe exploration (artifact `9dcf1d3a`); organizer landing (Shop Link) stays a mock. **White-label readiness = issue #45** (self-contained `/shop` + tagline/accent columns; embeds ruled out — hosted white-label only; payouts manual until a stranger's store sells). Two warm store prospects (Pilates studio, bakery) — not yet asked.
