...
**OPEN — PR #95 UNMERGED (merge first next session): back design on the /d buy page + SECURITY tightening.** https://github.com/nicolovejoy/prntd/pull/95 — `canUseAsPlacementSource`'s thread allowance granted regardless of ownership (a cross-owner buyer could forge a private image id from the seller's thread, reachable via addToCart); now requires owning the design, tested. Plus "Add a back design (+$8)" on /d (`getBuyPageBackSourceGroups`: My Designs + Shop; This-design only for owner), `buyPublishedDesign` optional `backImageId`. 622 tests + `check` job green; e2e job failed once at 10m49s during the GitHub 503 outage (logs unfetchable, pattern says infra) — rerun e2e, verify, merge with `gh pr update-branch` + `--squash --admin`. Merge-at-will was Nico's standing instruction; e2e is not a required check but don't merge a money-path PR without a green or explained e2e.

**Next builds after #95: #59 cancel generation, #61 back-design discoverability** (partially addressed: #72 shipped cross-thread back sources — This design / My Designs / Shop — and #95 adds /d; #61's remaining gap is advertising the toggle). **Nico wants to place the first real two-sided prod order** (front from one thread, back from another via the picker) once #95 is live — hand him the smoke steps. Cross-thread caveat: other threads offer their PRIMARY image only. Open product calls: (1) should remembered product yield to pinned presentation on /d? (2) /shop store panels stay organizer-fixed placements. Verify on prod once: /design empty state on a phone (heading + always-visible chips, from PR #79).
