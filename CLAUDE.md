...
- **2026-07-04 multi-agent code review** (idempotency/architecture/coverage/test-quality) filed the debt backlog #37–#41. Architecture verdict: modular monolith, explicitly NOT microservices; lib import graph is acyclic; fulfillment is the extraction-ready seam if ever needed. Status:
  ...
  - **Remaining, in build order — re-scoped by the 2026-07-18 audit:** **#40** generation races is **2 workstreams, not 3**: (a)+(b) are ONE fix — non-atomic read-modify-write of `design.generation_count` (which seeds the R2 key `designs/{id}/{n}.png`), so concurrent Generates overwrite each other's R2 object (permanent image loss) AND undercount the counter; fix = atomic increment returning the reserved number, key off that. (c) quota raciness REFUTED (`bump` upsert is atomic `count+1`) — only DB-path test coverage is missing. **#41** test debt — every item verified still open 2026-07-18 (seed factories, node env for lib tests, `orderBy` on order_item select, e2e data-testids beyond chat messages, cart pricing unit tests, vacuous `ai.test.ts` merge assertion). Then data-model **Phase 1b** (always write `order_item`; also removes #38's single-item caveat) → **1c** (drop scalar cols).
  - **2026-07-18 three-agent audit (money-path / generation-loop / roadmap-drift) → money-safety plan WP1–WP4, in flight.** New money-path findings: (1) **HIGH** — `order_canceled` webhook books a `refund` ledger row but nothing anywhere calls `stripe.refunds.create`; customer stays charged while books claim a refund. Decision (Nico 2026-07-18): **admin-clicked refund button**, never auto-refund from a webhook. (2) **MED/HIGH** — a DB failure after successful Printful submission leaves the order `paid` with no `printfulOrderId`; every cron/admin retry is then rejected by the `external_id` dedupe forever (COGS unbooked, shipping webhook can't match, no tracking email). Fix: on duplicate rejection, fetch the Printful order by `external_id` and persist id+COGS. (3) **MED** — `refund_cogs_reversal` is declared in schema but emitted nowhere; submitted-then-canceled orders understate gross profit. (4) throws after the paid-claim (image resolution runs before `submitOrderFulfillment`'s try; cart-clear loop uncaught) → 400 → Stripe redeliveries skip (status≠pending) → cron-only recovery, up to 24h lag. (5) `itemPrice` reconciles to pre-discount `amount_subtotal`, so promo orders store `itemPrice+shippingPrice > totalPrice`. Generation-loop extras beyond #40: quota consumed but not refunded on failed generation; orphaned R2 objects on post-upload failure; un-batched 4-write success path; leftmost `x-forwarded-for` is client-spoofable (IP cap weak). Plan: **WP1** refund/cancel correctness (findings 1–4) · **WP2** #40 atomic-counter fix + quota DB tests · **WP3** #41 mechanical batch · **WP4 SHIPPED (PR pending)** Phase 1b — every checkout writes `order_item` (removes #38's single-item caveat) + consistent promo `itemPrice` split (finding 5: `itemPrice = amount_total − amount_shipping`, post-discount). Each WP = one subagent in a worktree → PR.
